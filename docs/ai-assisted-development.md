...
아래 사례는 이전 개발 단계의 기록입니다. 당시의 512MB 머신·실버 구조와 현재 구현을 구분하며, 최신 구성은 [Architecture](architecture.md), 검토 기준은 [동기화 기록](source-sync.md)에 정리했습니다.
...
```text
스코어보드 캐시 적중 기록: 28번 중 6번 (단순 비율 약 21.4%)
절감의 70%: DB 읽기 경로
절감의 30%: 캐시
```

기존 문서의 18%는 함께 적힌 6/28과 일치하지 않아 단순 비율을 바로잡았습니다. 이 수치는 현재 운영 캐시를 다시 측정한 결과가 아닙니다.
...
## Current design: 원문을 남겨 재조립하기

최신 구현은 경기 상세의 완성 응답만 저장하지 않습니다. 전체 경기 원문을 R2에 두고, 화면 응답은 버전과 함께 DB에 저장합니다. 응답 구조가 바뀌면 원문으로 다시 조립하고, 저장된 원문이 없는 경기만 원천으로 보완합니다.

이 구조에서 검토할 것은 “캐시가 빠른가”뿐 아니라 “응답을 바꿨을 때 같은 원천 데이터를 다시 받아야 하는가”입니다. 투구 마트 역시 실버를 재사용하고, 최신 등판만 원천으로 보완합니다. **재생성 가능한 재료와 바로 읽을 결과를 분리하는 것**이 두 구현의 공통 판단입니다. 이는 코드에서 확인한 설계이며 새로운 성능 측정 사례를 뜻하지 않습니다.
...
3. **Memory** — 웹·배치 각각의 메모리 한도 안에 들어오는가? 현재 창고 머신의 1GB 설정에서도 배치가 불필요하게 데이터를 통째로 들고 있지 않은가?
